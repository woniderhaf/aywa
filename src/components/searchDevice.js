...
export default class SearchDevide extends Component {
  ...
  scanNearByDevices = (seconds = 20) => {
    BleManager.scan([], seconds, false).then(results => {
  ...
  repeatSearch = () => {
    if (this.state.isBluetooth) {
      this.setState({loading: true, error: null, title: 'Поиск устройства'});
          ...
          {this.state.isBluetooth ? (
            <View style={styles.center}>
              ...
              {this.state.loading == true ? (
                <SvgXml xml={icons.loading} style={{position: 'absolute'}} />
              ) : (
                <SvgXml xml={icons.error} style={{position: 'absolute'}} />
              )}
            </View>
          ) : (
