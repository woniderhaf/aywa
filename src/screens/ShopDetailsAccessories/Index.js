...
// globals
import {API, MAPS, rarity} from '../../globals/Сonstants';
...
export default class ShopDetailsAccessoriesScreen extends Component {
  ...
  componentDidMount = async () => {
    ...
    App.prepare(this.props.navigation, async user => {
      ...
      console.log({WLKN});
      const dataMats = await Storage.get('mats');
      this.setState({
        user,
        loading: false,
        ...
        <Template
          ...
          headerContext={
            <View style={[s.tab, s.center, {height: 40}]}>
              ...
              <Text style={[styles.white]}>{data.number}</Text>
            </View>
          }>
            ...
            <View style={s.container}>
              ...
              <ScrollView contentContainerStyle={s.item}>
                ...
                <View style={s.wrapperAccessories}>
                  {this.state.dataMats.slice(0, 4).map((v, i) => (
